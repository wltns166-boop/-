...
2. **Anthropic API 키를 Firebase 시크릿으로 등록** — 로컬 PC에서 한 번만 실행:
   ...
   ```
3. **`firebase.json`에 함수 연결(rewrite)과 functions 설정을 다시 추가** — 호스팅 배포를
   안정화하기 위해 현재는 빠져 있습니다. 함수를 쓸 때 `hosting` 안에 rewrites를 넣고
   `functions` 블록을 추가하세요:
   ```json
   {
     "hosting": {
       "public": ".",
       "ignore": ["firebase.json", ".firebaserc", "**/.git/**", "**/.github/**", "**/node_modules/**", "functions/**", "*.md", "insurance-intranet-v2.html"],
       "rewrites": [{ "source": "/api/**", "function": "api" }]
     },
     "functions": { "source": "functions", "runtime": "nodejs20" }
   }
   ```
4. 로컬 PC에서 `firebase deploy --only functions` 로 함수를 배포합니다.

> 함수 코드는 `functions/index.js`에 있으며, API 키는 브라우저에 노출되지 않고
> 서버에서만 사용됩니다. 현재 자동 배포 워크플로는 **호스팅만** 배포합니다(가장 안정적).
