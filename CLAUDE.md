...
- **입력**(9칸, 전부 원 단위 자동 쉼표 `_ssFmtInput` + 아래 한글 환산 `_ssKor` "15억 3,000만원"): 부동산/금융재산/기타재산/
...
- **상속 vs 증여 비교 (2026-09-15 v-8)**: 입력칸 [자녀 1인당 미리 증여할 금액](`gper`)에 값을 넣으면 결과 아래
  `_ssCompareHtml`이 "지금 그대로 상속" vs "미리 증여 후 상속"을 같은 입력으로 비교.
  `_ssGiftTax(금액, 공제)` — 증여세도 같은 `SS_RATES`, 공제는 `SS_GIFT_DED`(배우자 6억/성년 5천만/미성년 2천만/기타 1천만, 10년 합산).
  `_ssCompare(o)` 가정: ① 증여 후 **10년 경과**(상속재산 미합산) ② 증여 재원은 **금융재산 먼저, 모자라면 부동산** ③ 자녀는 성년·10년 내 다른 증여 없음.
