...
- **계약(작성) 일자·출력 (2026-08-21, 4차)**: 계약 줄 아래 년/월/일 분리 입력(`sd_wy/wm/wd` — 년 칸에 YYYYMMDD 한 번에
  입력하면 자동 분배 `sdDateInput`) + [오늘자 계약] 체크(`sd_today` — 체크 시 오늘 자동, 손으로 고치면 해제, 진입 시 기본 체크).
  값은 `base.wDate`('YYYY.MM.DD')로 전달 — **drawClaimFields·ccBuildCtx의 ctx가 wDate 있으면 작성일(yy/mm/dd)로 사용**
  (복붙 두 곳 동시 수정, 청구건에는 wDate가 없어 기존 '오늘' 동작 그대로). 일부만 입력·범위 밖·달력에 없는 날짜(2/31)면 생성 중단+안내.
  년 칸은 maxlength 없이 JS로 8자리 제한('2025-02-05' 구분자 붙여넣기도 숫자만 추려 분배 — maxlength가 DOM에서 먼저 잘라 씹히던 문제).
