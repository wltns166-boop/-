...
- 작업 브랜치: `claude/tims-intranet-continuation-q1rowj` (2026-09-04부터 — 세션 접미사 브랜치. 이전: claude/tims-gs-auto-deploy-ih0edz-g13v3j(12d1e53) → claude/tims-gs-auto-deploy-ih0edz(fe1d72b에서 분기, 그 이전: claude/insurance-customer-registration-button-c909l6 → claude/team-tops-intranet-continue-ozkg5p → claude/team-tops-handoff-prompt-o9j855 → claude/insurance-claim-document-reuse-fdje8i))
  ⚠️ 새 세션은 **원격 `claude/tims-intranet-continuation-q1rowj`의 최신 커밋** 위에서 시작할 것 — `claude/**` 푸시는 즉시 라이브 배포되므로 낡은 브랜치(g13v3j의 12d1e53, fe1d72b의 ih0edz 등)에서 푸시하면 옛 코드가 배포된다.
- 대화·주석은 **한국어**로.
...
- 관리자가 홈 진입(rHome) 시 역할별 처리 대기 건수 팝업(`#reqstat_pop`, 접속당 1회 — cwvac 패턴·`window._reqStatShown`,
  ?rr=1·미로그인 제외). **지점장(BM=_isAdmin&&!_isChongmu)**: 출장보고(최근 30일 확인 대기)/요청관리(검토중, 명함신청 포함)/
...
- **리뷰 반영(2026-09-08)**: ① 새 기기 오탐 방지 — 클라우드 미수신(_fsLoaded 전)+전부 0건이면 표시 보류·플래그 안 세움
  ...
  (팝업이 열려 있으면 수신 시 갱신, 닫았으면 그 세션엔 재표시 안 함 — 페이지 진입하면 실데이터 표시).
