...
- **특약 매핑 관리 (2026-08-13)**: [특약 매핑 관리] 버튼(관리자 `.ao`) → 모달(`waMapMgrOpen`)에서
  ...
  별도 `mapped:[{row,c,v}]` 배열로 출력(규칙 14가 행 번호를 "row N"으로 명시), `_waMergeMappedEdits`가 매핑 행의 AI 자유 기입(edits)을
  버리고 mapped 값을 규칙 행×상품 열(홀수 열은 왼쪽 앵커로 교정)에만 배치 — 매핑 행 오배치 원천 차단. mapped 미출력(구응답)이면
  기존 동작 폴백. 매핑 규칙이 없으면 출력 형식에 mapped 자체가 없음. 행 탐색은 `_waGridRowOf`(라벨 열 0~5 정확 일치).
