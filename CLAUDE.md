...
- **같은 내용은 한 줄로**: `rCustList`가 지문 `_custFp(c)`(이름·주민번호·담당자·지역·특이사항·가족 구성 —
  가족은 이름+주민번호+관계를 정렬해 이어 붙임, 주민번호 하이픈·공백 무시)로 묶어 **대표 1건만 그린다**.
  대표 행 이름 옆에 `중복 n건` 배지 + [정리](`custDupClean`) 버튼.
  ⚠️ **주민번호가 없으면 `_custFp`가 빈 문자열을 돌려줘 절대 합치지 않는다** — 이름만 같은 동명이인이
  한 줄로 합쳐지면 남의 기록이 화면에서 사라지고 아래 삭제까지 함께 되는 사고가 난다. 되돌리지 말 것.
  내용이 하나라도 다르면(특이사항·지역·가족 등) 따로 표시 — 잘못 합치는 쪽보다 두 줄로 보이는 쪽이 안전하다는 방침.
  가족 행도 `_famFp(f)`(이름+주민번호+관계)로 같은 규칙.
...
- **삭제는 합쳐 보이던 건을 함께 지운다**(`deleteCustRow` → `_custDupIdxs`, `deleteFamRow`) —
  첫 건만 지우면 남은 복제가 그 자리를 채워 **"삭제가 안 된 것처럼" 보이기 때문**. confirm 문구에 건수를 명시하고,
