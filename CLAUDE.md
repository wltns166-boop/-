...
- **현황 표 주차별 작성여부 (2026-08-18)**: 표2(팀원별 미팅 현황)에 주차 드롭다운(`mt_week_sel`)+[주차 작성여부] 열(`mt_wk_th`) —
  ...
  주차 배지는 클릭 가능(`mtWkWrite(nm,wk)` — 이름+주차키, 함정 A): 표1 팀원 선택을 그 팀원으로 바꾸고(미저장 가드 `_mtGuardUnsaved` 경유,
  드롭다운에 없는 이름은 토스트) 그 주차 피드백 입력행을 열어 포커스+스크롤. 저장 권한 검증은 기존 mtFbSave가 담당.
