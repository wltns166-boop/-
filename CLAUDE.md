...
- **폼 개편 (2026-08-21, 2차)**: 계약자·피보험자는 **성명만** 입력(주민번호/연락처/주소 칸 제거 — [계약자 동일] 버튼 유지),
  모집자 성명은 로그인 사용자 자동. 계약 정보는 **동적 줄**(`#sd_ct_rows` — 번호. 보험회사/상품명/증권번호 + ✕,
  [＋ 계약 추가]=`sdRowAdd`, 줄 조작은 closest 기반 — 함정 A 안전, 마지막 줄은 삭제 대신 값 비움).
  생성은 **계약(줄)×문서 이중 루프** — 값 있는 줄마다 선택한 문서 세트를 만들어 한 PDF로 병합(빈 줄뿐이면 빈 계약 1건),
  양식은 문서별 1회만 조회(`tplMap`). 좌표 편집기 3종 모드 도구박스·항목 추가 드롭다운은 `CC_SAM_KEYS` 화이트리스트
  (폼에 입력칸 있는 값만 — 주민번호·휴대폰·주소·설계번호·관계 숨김, 배치돼 켜진 항목의 칩은 계속 표시됨).
