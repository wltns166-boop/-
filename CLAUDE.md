...
- **지도 이미지·명함 첨부 + [공유하기로 보내기] (v-6, 사용자 선택 (가))**: 왼쪽 [지도 첨부]·[내 명함] 칸.
  ...
  · [공유하기로 보내기](`rsvShare`): `navigator.share({text, files:[지도, 명함]})` — 휴대폰 공유 창에서 문자 앱 선택 → MMS 작성 화면.
    ...
    **v-7 리뷰**: 캡처 뒤 장소를 바꾼 상태(`_rsvMapImg.q`≠현재 장소)면 공유 전에 confirm — 확인=지도 빼고 보냄/취소=중단
    (옛 장소 지도+새 장소 문구가 함께 나가 고객이 약속 장소를 착각하는 경로 차단). 화면 캡처 대체 경로도 캡처 중 장소 변경 시 버림.
  · ⚠️ 수용(기존 구조적 한계 계열): `/api`는 인증이 없고 `_smHits` 분당 상한은 인스턴스 메모리 기준이라 남용 방어가 약함 —
    유료 전환이 걱정되면 NCP 콘솔 사용량 한도·알림을 걸 것(Firestore 전역 카운터 강화는 별도 과제).
