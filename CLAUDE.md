...
- 목록(rExam)은 관리자=전체, 비관리자=**본인이 등록(submitter)한 건 + 소속(belong)이 본인인 건**.
  belong 조건은 2026-09-22 실사고 반영 — 관리자가 팀장 몫을 대신 등록하면(submitter=관리자) 그 팀장 화면에
  "신청 내역이 없습니다"로 나오던 문제("신청했는데 리스트에 안 올라옴" 제보. 클라우드 저장은 정상이었음 — 표시 필터 문제).
