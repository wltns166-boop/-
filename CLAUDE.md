...
- **폼 개편 (2026-08-21, 2·3차)**: 계약자·피보험자 **성명만 한 줄 반반** 입력(주민번호/연락처/주소/서명 캔버스 제거 —
  ...
  건수 합만큼 계약 줄이 자동 생성·삭제(`_sdSyncRows` — 뒤에서만 추가/제거로 앞줄 입력 보존, 줄마다 소속 계약자 라벨 `.sdown` 표시).
  줄은 순서대로 계약자에 소속(1번 계약자 2건 → 1·2번 줄) — 생성 시 그 줄의 custName으로 사용, 빈 줄은 건너뛰되 소속은 위치 기준.
  계약 줄 직접 추가/삭제 버튼은 제거(건수가 줄 수를 결정). **문서 선택 UI 없음** — 계약(줄)마다 자동 세트: 개인정보처리동의서+고지의무확인서,
