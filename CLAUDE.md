...
- **작성일·청약일·출력 (2026-08-21, 4·7차)**: 전역 계약(작성) 일자 입력·[오늘자 계약] 체크는 **제거됨**(7차) —
  작성일의 년·월은 생성일 자동, **일(日)은 계약 줄별 [청약일(일)] 칸**(`data-sd-day`, 1~31 검증·2자리 패딩,
  **비우면 빈칸으로 출력**해 자필 기입). 값은 `c.wDay`(''=빈칸)로 전달 — **drawClaimFields·ccBuildCtx의 ctx.dd가
  wDay 정의 시 그 값 사용**(복붙 두 곳 동시 수정, 청구건에는 wDay가 없어 기존 '오늘' 그대로). 엔진의 `c.wDate`(전체 날짜
  오버라이드, 'YYYY.MM.DD') 지원은 남아 있으나 현재 보내는 곳 없음.
