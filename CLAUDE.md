...
- 문구 틀 치환 `_rsvFill(mode)`: `{성함}` `{담당자}`(=로그인 이름 `cu.name`) `{상담일시}`(`_rsvWhen` → "9월 27일 일요일 14시 30분", 날짜는 로컬 생성 —
...
- [설정] 모달 `m_rsvset`: 문구 틀 편집(토큰 삽입 버튼·[기본 문구로]). 저장 키 **`tops_rsvcfg` = {tpl:{사용자이름: 문구}}** (사용자별, 텍스트만 — 함정 B).
