...
- 작업 브랜치: `claude/team-tops-intranet-continue-phg1ra` (2026-08-05부터. 이전: claude/team-tops-intranet-continue-bfgyd4)
- 대화·주석은 **한국어**로.
...
- 카드: `{id('rc_<ts>_<rand>'), owner, ts, dt, ut, vals:{항목id: 값|[값...]}, prints:[{by,dt}]}`.
  탐색·수정은 **항상 카드 id**(`window._rcCur`) 기준(함정 A). ◀▶ 넘김·빠른이동 셀렉트·[페이지 추가].
...
- **내용 페이지 게시판 (2026-08-05)**: `cm_*` 페이지는 **게시판이 주 화면** — 목록은 대표이미지 썸네일+제목 카드 그리드
  ...
  **파일 접근 권한**: 팀원=인라인 열람만(원본 링크·PDF 버튼 없음, 우클릭/드래그 억제 — 완전 차단은 불가, 억제 수준),
  관리자=이미지 클릭 원본·[원본 PDF] 다운로드. **대표이미지(썸네일)**: `post.cover`(작성 모달 [대표이미지 등록], 파일 추가 위)
...
8. 커밋 후 `claude/team-tops-intranet-continue-phg1ra` 로 푸시. (⚠️ `claude/**` 브랜치는 푸시 즉시 GitHub Actions가 라이브 배포함)
